...
from http_util import (b, bytes_to_str, IOrderedDict, StringIO,
        unquote, MAXSIZE)
...
class HttpParser(object):
        ...
        while True:
            if not self.__on_firstline:
                ...
                try:
                    ...
                    if self.__on_headers_complete:
                      # Return when headers are parsed
                      return nb_parsed
    ...
    def _parse_firstline(self, line):
        try:
            ...
            elif self.kind == KIND_RES:
                self._parse_response_line(line)
            elif self.kind == KIND_REQ:
                self._parse_request_line(line)
